...
abstract class BaseController extends Controller {
	...
	public function __construct($appName,
			IRequest $request, ILogger $logger, ResponseFactory $responseFactory) {
		...
		foreach($reflectionClass->getMethods(\ReflectionMethod::IS_PUBLIC) as $method) {
			...
			if(strstr($docs, '@ControllerManaged') !== false) {
				throw new \OCA\EasyBackup\EasyBackupException('ContollerManaged method "' . $method->getName()
						. '" in class  ' . $reflectionClass->getName() . ' is not allowed to be public');
			}
		}
	}

...
class Application extends App {
	...
	public function __construct(array $urlParams=array()){
		 ...
		 * Controllers
		 */
		$this->getContainer()->registerService('PageController', function($c) {
			return new PageController(
					...
					$c->query('ScheduleService'),
					$c->query('URLGenerator'),
					$c->query('ResponseFactory')
			);
		});

		$this->getContainer()->registerService('ConfigController', function($c) {
		...
		$this->getContainer()->registerService('BackupController', function($c) {
		...
		$this->getContainer()->registerService('LogfileviewController', function($c) {
			return new LogfileviewController(
					$c->query('AppName'),
					$c->query('Request'),
					$c->query('Logger'),
					$c->query('ConfigService'),
		 ...
		 * Services
		 */
		$this->getContainer()->registerService('BackupService', function($c){
			return new BackupService(
					$c->query('RunOnceJob'),
					$c->query('ScheduledBackupJob'),
					$c->query('ConfigService'),
					$c->query('ShellExecService'),
		...
		$this->getContainer()->registerService('TranslationService', function($c){
			return $c->query('ServerContainer')->getL10N($c->query('AppName'));
		});

		$this->getContainer()->registerService('ShellExecRunnable', function($c){
		...
		$this->getContainer()->registerService('ConfigService', function($c){
			return new ConfigService(
					$c->query('AppName'),
					$c->query('OwncloudConfig')
			);
		});

		$this->getContainer()->registerService('Logger', function($c) {
			return $c->query('ServerContainer')->getLogger();
		});

		$this->getContainer()->registerService('OwncloudConfig', function($c) {
			return $c->query('ServerContainer')->getConfig();
		});

		$this->getContainer()->registerService('URLGenerator', function($c) {
			return $c->query('ServerContainer')->getURLGenerator();
		});

		$this->getContainer()->registerService('ScheduleService', function($c){
